...
class TodoList {
  ...
  getNoteByName(searchText) {
    this.notes.forEach((note, index) => {
      if(note.text === searchText) {
      console.log(`${index}. Status: ${note.note_cmplt_st}, task: ${note.text}`);
      } 
    });
  }

  sortNotesByStatus(sortParam = true) {
    this.notes.sort( (a, b) => {
      if (a.note_cmplt_st === b.note_cmplt_st) {
        return 0;
      }
      if (sortParam) {
        return b.note_cmplt_st
      } else {
        return a.note_cmplt_st
      }
    });
  }
...
console.log('--------- find ---------');
console.log(myTodoList.getNoteByName('Walk the dog'))
console.log('--------- sort ---------');
console.log(myTodoList.sortNotesByStatus(false)); 
console.log(myTodoList.getAllNotesByList())